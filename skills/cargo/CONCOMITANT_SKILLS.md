...
- `flecs-dylib-modules` — invoked by — Hot reloadable Flecs modules as Rust dylibs.
...
- `graft-prefer` — sibling in the reproducible environments & deployment cluster — > <<<<<<< HEAD Enforces graft as the preferred build system for fully covariant computati…
...
- `rust` — invoked by — Rust ecosystem = cargo + rustc + clippy + rustfmt.
...
- `signal-messaging` — invoked by — Send and receive Signal messages via MCP.
