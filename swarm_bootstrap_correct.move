...
module zubyul::swarm_bootstrap {
    ...
    public fun generate_order_id(
        ...
        let mut stream = derive_stream(master_seed, wallet.letter_index, trit_xorbyte);
    ...
    public fun create_triadic_order_set<M: store + copy + drop>(
        ...
        let mut minus_stream = derive_stream(master_seed, wallet.letter_index, MINUS_XORBYTE);
        let mut ergodic_stream = derive_stream(master_seed, wallet.letter_index, ERGODIC_XORBYTE);
        let mut plus_stream = derive_stream(master_seed, wallet.letter_index, PLUS_XORBYTE);
    ...
    public fun verify_gf3_conservation(wallet_count: u8): bool {
        ...
        let total_trit_sum: i32 = (26 as i32) * (-1 as i32)
            + (26 as i32) * (0 as i32)
            + (26 as i32) * (1 as i32);
    ...
    fun test_splitmix64_determinism() {
        let seed = 1069u128;
        let mut rng1 = splitmix64_new(seed);
        let mut rng2 = splitmix64_new(seed);
