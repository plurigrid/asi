...
## Ordered Locale Support

ACSets provide the **frame structure** for ordered locales (Heunen-van der Schaaf 2024):

```julia
using Catlab.CategoricalAlgebra.Subobjects: ThSubobjectHeytingAlgebra

# Subobjects of an ACSet form a Heyting algebra (frame!)
# This gives us the lattice operations: ∧, ∨, →, ¬

@present SchOrderedLocale(FreeSchema) begin
    Open::Ob           # Opens in the locale
    Point::Ob          # Points (frame homs L → 2)
    
    mem::Hom(Point, Open)        # x ∈ U
    leq::Hom(Point⊗Point, Bool)  # x ≤ y (preorder)
    
    # Cone operators (must preserve opens)
    up_cone::Hom(Open, Open)     # ↑U
    down_cone::Hom(Open, Open)   # ↓U
end

# The open cone condition: ↑U and ↓U must be in Opens
function verify_open_cones(L::ACSet)
    for U in parts(L, :Open)
        up_U = L[U, :up_cone]
        down_U = L[U, :down_cone]
        @assert up_U in parts(L, :Open) "↑$U not open"
        @assert down_U in parts(L, :Open) "↓$U not open"
    end
end
```

### Cones and Cocones via Catlab

```julia
using Catlab.CategoricalAlgebra: limit, colimit, cone, cocone

# Limit cone in ACSet category
lim = limit(diagram)
apex(lim)           # The limit object
legs(cone(lim))     # Projection morphisms

# Colimit cocone
colim = colimit(diagram)  
apex(colim)         # The colimit object
legs(cocone(colim)) # Inclusion morphisms

# For ordered locales: cones must preserve order
function ordered_limit(L::OrderedLocale, diagram)
    lim = limit(diagram)
    # Verify: if i ≤ j in diagram, cone legs preserve this
    verify_cone_order(lim, L)
    lim
end
```

See: `~/.claude/skills/ordered-locale/` for full implementation.
See: `~/worlds/ordered-locale/Catlab.jl/src/categorical_algebra/cats/Subobjects.jl`
